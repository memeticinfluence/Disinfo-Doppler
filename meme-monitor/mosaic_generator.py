...
def main(input_date, subreddit):
    ...
    if os.path.exists(new_dir):
        ...
        records = ps.download_subreddit_posts(
    ...
    dataset = Feature_Extraction_Dataset(
        _df_img_meta, img_col="f_img", index_col="d_hash"
    )
    ...
    for (X, img_file, idx) in data_loader:
        ...
        if filt:
            ...
            df_conv = pd.DataFrame.from_dict(
                logits_dict, columns=cols_conv_feats, orient="index"
            )
    ...
    # Model files
    file_encoder = (
        f'{ config.working_dir }/encoders/{ str(min_dist).replace(".", "-") }_'
    ...
    if not os.path.exists(file_encoder) or overwrite_model:
        ...
        if not os.path.exists(file_training_set):
            training_set = conv[config.cols_conv_feats].sample(
                training_set_size,
                replace=training_set_size > conv.shape[0],
                random_state=303,
            )
        else:
            training_set = pd.read_csv(file_training_set, index_col=0)
    ...
    if not os.path.exists(logits_dir):
        ...
        df_merged = pd.read_csv(logits_dir, index_col=0, compression="gzip")

    tile_width, tile_height = (
        config.tile_width,
        config.tile_height,
    )  # pixel dimenstions per image
    ...
    draw.text(
        (4, (tile_height * (ny)) + 10), f"{subreddit}_{input_date}", (229, 229, 229)
    )

    mosaic.save(
        f"django-app/MemeWebsite/main/static/mosaics/{subreddit}_{input_date}.png"
    )
